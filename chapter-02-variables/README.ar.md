...
- **المسار الأساسي · 45–60 دقيقة:** الأقسام 1 و2.1 و3 و4 و5.1–5.5 و7–9 وcheckpoint الأخير. النتيجة: سكربت ملف شخصي صغير بمتغيرات واضحة ونص نظيف وحسابات، من دون `if` أو `def` أو `try` أو `raise`. تنتهي عندما ينتج حالتا الاسم العادي والفارغ المخرجات المحددة وتستطيع التعافي من `NameError` المقصود؛ عندها يمكنك التوقف بأمان أو المتابعة إلى الفصل 3.
...
يكتمل المسار الأساسي عند 5/5. لا يحتاج إلى شروط أو دوال أو معالجة استثناءات أو اختبارات.

---

## تأمل ختامي

أي توقع تغيّر بعد تشغيل الحالات العادية والحدّية والمُصلحة؟ اشرح لماذا تستطيع عبارة `strip()` نفسها تنظيف اسم ظاهر واسم مكوّن من مسافات فقط من دون `if`.

تستطيع الآن شرح عمل المفسّر، واستخدام المتغيرات كملصقات، وتنسيق النصوص وتنظيف المسافات، وإجراء العمليات الحسابية، والتعافي من خطأ في الاسم. وتعرف أيضًا روح Zen of Python: اجعل الحل بسيطًا وقابلًا للقراءة. في **الفصل 3** سنخزّن مجموعات كاملة من البيانات باستخدام **lists** ونتعلم قراءتها وتعديلها وترتيبها. احتفظ بهذه الأمثلة قريبًا؛ سنستخدمها مجددًا.
