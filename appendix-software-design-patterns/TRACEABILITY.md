...
| C-P3 | recognize Command, Singleton, Proxy, and Template Method without implementing them | [decision cards](README.md#professional-decision-cards) | full decision record for each card | justified use; simpler alternative; removal threshold | Professional card review | prose/manual review; status `decision-card` |
...
| C-A3 | assess Facade and defer Repository/service layer to owners | [Facade card](README.md#facade-decision-card-and-owned-cross-links) | decision record plus Chapter 17/28 reading | justified card; no duplicate implementation; removal case | Advanced card review | prose/manual, Chapters 17/28 cross-links |
...
and the named suite; cards/cross-links require manual explanation rather than a
new implementation.
...
| Factory Method | decision-card contrast | Essential | Appendix C prose |
| composition Decorator, bounded synchronous Observer | executable | Professional | Appendix C core suite |
| Command, Singleton, Proxy, Template Method | decision-card | Professional | Appendix C prose |
| dependency injection, composition root, execution port, in-memory Adapter | executable | Advanced | Appendix C core suite |
| Facade | decision-card | Advanced | Appendix C prose |
| Retry and Circuit Breaker/State | executable | Resilience | Appendix C network suite |
...
|---|---|---|---|
| direct, Essential, Professional, Advanced excerpts | `appendix-software-design-patterns/examples/patterns/{direct,essential,professional,architecture}.py` | `patterns:core-suite` | sources exist and 21 focused tests pass; shared known-check ownership must still accept every excerpt before publication |
| Resilience, Capacity, Pub/Sub excerpts | `appendix-software-design-patterns/examples/patterns/{resilience,capacity,pubsub}.py` | `patterns:network-suite` | corrected sources pass 47 focused tests on CPython 3.11.14/3.12.12/3.13.11/3.14.2 and independent adversarial review; shared known-check acceptance remains pending |
...
The present Appendix C test paths are
`examples/tests/test_core_patterns.py` and
`examples/tests/test_network_patterns.py`. Generic check execution is bounded
...
21 tests and network's corrected 47 tests are green on CPython
3.11.14/3.12.12/3.13.11/3.14.2; the complete appendix suite is 68/68. An
...
plugin's seven synthetic selection/failure fixtures and real core/network
