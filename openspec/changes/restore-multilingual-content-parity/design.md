...
- **Paquete de paridad:** `tools/parity_manifest.json` conserva únicamente el índice de topología; `tools/parity/sources/<unit>.json` y `tools/parity/records/<unit>/<locale>.json` son los registros persistentes granulares con digests, mapa de cobertura, excepciones, resultados automáticos y decisiones humanas independientes. El paquete de capítulo se genera como una proyección read-only de una fuente, sus cuatro registros y las entradas aplicables de `ATTRIBUTIONS.toml`, referenciadas por ID y digest normalizado de entrada/evidencia; no se guarda como segunda autoridad ni almacena datos personales.
...
- `root_leaf_sha256` y la lista ID/digest de sus decisiones, ordenada por ID;
...
- `source.audit` se sustituye por `canonical_review`; `pending-human-review` queda pending y un legacy `approved` sin rol/fecha también vuelve a pending porque no aporta evidencia suficiente. Se añade `rendered_accessibility_review` pending.
- Se conservan `linguistic_review` y `technical_review` solo cuando su schema y ambos digests siguen válidos. Se añaden `rendered_accessibility_review` pending y, solo para `ar`, `bidi_review` pending.
...
- Tras el exchange, el sibling de staging conserva temporalmente el store v1. Solo se elimina después de volver a cargar y validar v2; si el reload falla, otro `RENAME_EXCHANGE` restaura v1 antes de limpiar el sibling. Si también falla ese rollback, se conserva ese recovery same-filesystem dentro del repositorio, se informa su path relativo sin afirmar éxito y la higiene permanece bloqueada hasta recuperación manual. No se deja backup ni staging tras una migración correcta.
...
`ATTRIBUTIONS.toml` sigue siendo la única autoridad de procedencia. Para cada entrada aplicable, el paquete calcula `provenance_sha256` sobre `canonical_json_bytes({inventory_schema_version, entry, evidence: [{path, sha256}, ...]})`: `entry` incluye todos los campos permitidos por el schema vigente, con keys canónicas y `paths` normalizados/ordenados; evidence se ordena por path. Un campo desconocido, path ausente o inseguro falla cerrado. El paquete registra ID/digest/status como evidencia derivada. Una entrada de capítulo bloquea solo las páginas que cubre; `LICENSE` y `icons/cc-by-sa.svg` son gates globales de publicación. Un cambio en schema, entrada o path invalida únicamente los paquetes que la referencian, mientras un diagnóstico root global puede seguir bloqueando la promoción automática completa sin convertir una entrada ajena de capítulo en gate semántico de otra unidad.
