...
يحوي الخطأ `version` و`type` و`code` مستقراً و`message` محدوداً فقط، ولا يعكس payload كاملاً. تتتبع كل وصلة 64 مستشعراً كحد أقصى. يعيد المستشعر 65 `resource_limit` دون طرد الحالة. يحدث التحقق قبل mutation، لذلك كل رفض transactional.
...
قيّد الوقت وbytes والعملاء والمستشعرات والخرج المعلّق. سجّل peer وفئة خطأ مستقرة، لا أسراراً أو payload كاملاً. أعد فقط العمليات الآمنة بعدد قليل وbackoff؛ لا يعيد المشروع الكتابة آلياً.
...
ينتظر الخادم المتسلسل داخل `recv()`. تسأل `selectors.DefaultSelector` نظام التشغيل أي sockets جاهزة. تقبل النسخة المرافقة 32 عميلاً كحد أقصى، وتحفظ 65,536 byte ناقصة و64 مستشعراً لكل اتصال، وترمّز رداً معلقاً واحداً في كل مرة.
...
ينفذ الأمر الأول 27 اختباراً من المكتبة القياسية. يجمع الثاني checks العامة مع `network:network-suite`. يمتلك plugin سلوك البروتوكول ودورة الحياة المحلية فقط؛ تمتلك أداة الجذر Markdown والروابط ومبدلات اللغة وإشارات accessibility والبنية اللغوية والنظافة.

## التحدي النهائي

1. **سهل:** أرسل ثلاث قراءات متزايدة؛ TODO: ثلاثة acks دقيقة؛ تلميح: ابدأ من الصفر.
2. **متوسط:** أرسل duplicate بين قراءتين صالحتين؛ أثبت أن `out_of_order` لا يمنع sequence أكبر لاحقاً.
3. **متقدم:** عميلان نشطان وثالث متوقف وسط frame؛ أثبت التقدم والإغلاق بلا مهام باستخدام events و`wait_for`، لا انتظاراً تخمينياً.
4. **Hero:** فعّل TLS الموثوق أو IPv6 المشروط، واكتب بالضبط ما اختبره جهازك.

يجمع الحل `AsyncTelemetryHub` و`send_readings()` و`ConnectionState` والاختبارات القائمة؛ لا يصنع بروتوكولاً ثانياً. اختبر acks وerror واحداً والحالة وtimeout والإغلاق، وكل العناوين loopback.

## معيار التقييم

امنح كل جانب 0 (غائباً) أو 1 (جزئياً) أو 2 (مثبتاً): البروتوكول الدقيق؛ حدود frame والوقت و32 عميلاً و64 مستشعراً والخرج؛ التعافي transactional؛ تقدم العملاء؛ loopback وTLS المتحقق؛ الاختبارات الحتمية؛ التنظيف والشرح. اثنتا عشرة نقطة أو أكثر من دون صفر في البروتوكول أو الحدود أو الإغلاق نتيجة قوية. الدرجة تغذية راجعة؛ حسّن سلوكاً قابلاً للملاحظة كل مرة.
